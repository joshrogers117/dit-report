...
const { computeDayTotals, computeProjectTotals, computeCumulativeTotals } = require('./calculations');
...
.btn { background: var(--surface); border: 1px solid var(--border); color: var(--text); padding: 7px 16px; border-radius: 6px; cursor: pointer; font-size: 13px; font-weight: 500; transition: all .15s; }
...
.day-tab { background: var(--surface); border: 1px solid var(--border); color: var(--text-dim); padding: 10px 24px; border-radius: 8px 8px 0 0; cursor: pointer; font-size: 14px; font-weight: 600; transition: all .15s; border-bottom: 2px solid transparent; }
...
.camera-card { background: var(--surface2); border: 1px solid var(--border); border-radius: 8px; padding: 16px; }
...
.day-summary-card { background: var(--surface); border: 1px solid var(--border); border-radius: var(--radius); padding: 20px; box-shadow: var(--shadow); cursor: pointer; transition: border-color .15s, transform .15s; }
...
function renderHeaderHTML(project) {
  ...
  return `<div class="report-header">
    ...
    <div class="header-grid">
      ...
      ${project.dit_email ? `<div class="header-item"><label>Contact</label><span>${esc(project.dit_email)}</span></div>` : ''}
...
function renderStatsBar(totals) {
  return `<div class="stats-bar">
    ...
    <div class="stat-card"><div class="stat-label">Total Frames</div><div class="stat-value">${fmtInt(totals.frames)}</div></div>
...
function renderCameras(cameras) {
  ...
  return `<div class="section">
    ...
    <div class="camera-cards">
      ${cameras.map(c => `<div class="camera-card">
        <h3>${esc(c.camera_name)} ${c.label ? `<span class="cam-label">${esc(c.label)}</span>` : ''}</h3>
        <div class="cam-specs">
...
function renderRollsTable(rolls, totals) {
  ...
  return `<div class="section">
    ...
    <table>
      <tr><th>Roll</th><th>Card Serial</th><th style="text-align:right">GB</th><th>Duration</th><th style="text-align:center">Master</th><th style="text-align:center">Backup</th><th style="text-align:right">Frames</th><th>Notes</th></tr>
      ${rolls.map(r => {
        if (r.is_break) return `<tr><td colspan="8" style="text-align:center"><span class="badge badge-break">${esc(r.roll_name)}</span></td></tr>`;
        return `<tr>
          ...
          <td class="mono" style="text-align:right">${fmtInt(r.frames)}</td>
      ...
      <tr class="summary-row">
        ...
        <td></td><td></td>
        <td class="mono" style="text-align:right">${fmtInt(totals.frames)}</td>
        <td></td>
      </tr>
    </table>
  </div>`;
...
function renderStandaloneHTML(project) {
  ...
  const daysData = project.days.map(d => ({
    ...
    cameras: d.cameras.map(c => ({
      name: c.camera_name, res: c.resolution, codec: c.codec,
      colorspace: c.colorspace, lut: c.lut, fps: c.fps, label: c.label
    })),
    rolls: d.rolls.map(r => ({
...
document.getElementById("themeToggle").addEventListener("click", e => {
  ...
  document.querySelectorAll("#themeToggle .btn").forEach(b => b.classList.toggle("active", b === btn));
});
document.getElementById("layoutToggle").addEventListener("click", e => {
  const btn = e.target.closest("[data-layout]"); if (!btn) return;
  currentLayout = btn.dataset.layout;
  document.querySelectorAll("#layoutToggle .btn").forEach(b => b.classList.toggle("active", b === btn));
  render();
});
document.querySelector(".day-tabs").addEventListener("click", e => {
  const tab = e.target.closest(".day-tab"); if (!tab) return;
  currentDay = tab.dataset.day;
  document.querySelectorAll(".day-tab").forEach(t => t.classList.toggle("active", t === tab));
  render();
});
...
function renderRollsTable(rolls,totals) { return '<div class="section"><div class="section-header">Card Offloads</div><table><tr><th>Roll</th><th>Card Serial</th><th style="text-align:right">GB</th><th>Duration</th><th style="text-align:center">Master</th><th style="text-align:center">Backup</th><th style="text-align:right">Frames</th><th>Notes</th></tr>'+rolls.map(r=>{if(r.isBreak) return '<tr><td colspan="8" style="text-align:center"><span class="badge badge-break">'+r.roll+'</span></td></tr>'; return '<tr><td class="mono" style="font-weight:600">'+r.roll+'</td><td class="mono" style="color:var(--text-dim)">'+r.serial+'</td><td class="mono" style="text-align:right">'+r.gb.toLocaleString(undefined,{minimumFractionDigits:2})+'</td><td class="mono">'+r.duration+'</td><td style="text-align:center">'+statusBadge(r.master)+'</td><td style="text-align:center">'+statusBadge(r.backup)+'</td><td class="mono" style="text-align:right">'+r.frames.toLocaleString()+'</td><td style="color:var(--text-dim);font-size:12px;max-width:200px">'+r.notes+'</td></tr>';}).join('')+'<tr class="summary-row"><td>WRAP</td><td></td><td class="mono" style="text-align:right">'+totals.gb.toLocaleString(undefined,{minimumFractionDigits:2})+'</td><td class="mono">'+totals.duration+'</td><td></td><td></td><td class="mono" style="text-align:right">'+totals.frames.toLocaleString()+'</td><td></td></tr></table></div>'; }
function renderRollsCards(rolls,totals) { return '<div class="section"><div class="section-header">Card Offloads</div><div class="rolls-cards">'+rolls.map(r=>{if(r.isBreak) return '<div class="roll-card is-break">'+r.roll+'</div>'; return '<div class="roll-card"><div class="roll-card-header"><h4>'+r.roll+'</h4><div>'+(r.master?'<span class="badge badge-ok">Master</span>':'')+(r.backup?'<span class="badge badge-ok" style="margin-left:4px">Backup</span>':'')+'</div></div><div class="roll-card-meta"><div><div class="cam-spec-label">Size</div><div class="cam-spec-value mono">'+r.gb.toLocaleString(undefined,{minimumFractionDigits:2})+' GB</div></div><div><div class="cam-spec-label">Duration</div><div class="cam-spec-value mono">'+r.duration+'</div></div><div><div class="cam-spec-label">Frames</div><div class="cam-spec-value mono">'+r.frames.toLocaleString()+'</div></div><div><div class="cam-spec-label">Card</div><div class="cam-spec-value mono" style="font-size:11px">'+r.serial+'</div></div></div>'+(r.notes?'<div class="roll-card-note">'+r.notes+'</div>':'')+'</div>';}).join('')+'</div><div style="padding:12px 20px;border-top:1px solid var(--border);background:var(--surface2);display:flex;gap:32px;font-weight:600;font-size:13px"><span>Total: <span class="mono">'+totals.gb.toLocaleString(undefined,{minimumFractionDigits:2})+' GB</span></span><span>Duration: <span class="mono">'+totals.duration+'</span></span><span>Frames: <span class="mono">'+totals.frames.toLocaleString()+'</span></span></div></div>'; }
function renderOverview() { return '<div class="all-days-grid">'+DAYS.map((d,i)=>'<div class="day-summary-card" onclick="document.querySelectorAll(\\'.day-tab\\')['+(i+1)+'].click()"><h3>'+d.label+'</h3><div class="day-date">'+d.dateFormatted+'</div><div class="day-stats"><div><div class="day-stat-label">Data</div><div class="day-stat-value">'+d.totals.gb.toLocaleString()+' GB</div></div><div><div class="day-stat-label">Duration</div><div class="day-stat-value">'+d.totals.duration.substring(0,8)+'</div></div><div><div class="day-stat-label">Rolls</div><div class="day-stat-value">'+d.rolls.filter(r=>!r.isBreak).length+'</div></div><div><div class="day-stat-label">Frames</div><div class="day-stat-value">'+d.totals.frames.toLocaleString()+'</div></div></div><div style="margin-top:12px;font-size:12px;color:var(--text-dim)">Drives: '+d.benchmarks.map(b=>b.name).join(", ")+' &bull; '+d.cameras.length+' cameras</div></div>').join('')+'</div>'; }
function renderDay(i) { const d=DAYS[i]; const r=currentLayout==="table"?renderRollsTable(d.rolls,d.totals):renderRollsCards(d.rolls,d.totals); return '<div style="margin-bottom:16px"><h2 style="font-size:20px;font-weight:700">'+d.label+' &mdash; '+d.dateFormatted+'</h2><div style="color:var(--text-dim);font-size:13px;margin-top:4px">'+d.totals.gb.toLocaleString()+' GB &bull; '+d.totals.duration+' &bull; '+d.totals.frames.toLocaleString()+' frames &bull; '+d.rolls.filter(r=>!r.isBreak).length+' rolls</div></div>'+renderBenchmarks(d.benchmarks)+renderCameras(d.cameras)+r; }
function render() { const el=document.getElementById("content"); el.innerHTML = currentDay==="overview" ? renderOverview() : renderDay(parseInt(currentDay)); }
render();
</script>
</body>
</html>`;
    ...
    return `<div class="day-section">
      <div class="day-divider">
        ...
        <div style="color:var(--text-dim);font-size:13px;margin-top:4px">
          ${fmtNum(d.totals.gb)} GB &bull; ${d.totals.duration} &bull; ${fmtInt(d.totals.frames)} frames &bull; ${d.totals.rollCount} rolls
        </div>
      </div>
      ...
      <div class="cumulative-bar">
        ...
        <div><div class="cum-label">Cumulative Frames</div><div class="cum-value">${fmtInt(cum.frames)}</div></div>
      </div>
    </div>`;
  }).join('');
...
module.exports = { renderStandaloneHTML, renderPrintHTML };
