...
:root {
  ...
  --shadow: none;
  --tr-caps: 0.14em;
  /* Categorical palette for per-source data marks. Validated for adjacent-pair
     CVD separation and normal-vision distinctness on both surfaces — the brand
     palette itself is too warm/muted to pass, so marks use these and the
     surrounding text/chrome stays brand ink. */
...
var SRC_COLOR = (function(){ var m = new Map(); DAYS.forEach(function(d){ daySourceSums(d).forEach(function(v, k){ if (!m.has(k)) m.set(k, (m.size % 8) + 1); }); }); return m; })();
