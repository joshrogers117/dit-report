...
@media print { .section, .day-divider, .cumulative-bar, .report-header, .stats-bar { page-break-inside: avoid; } tr { page-break-inside: avoid; } }
...
function renderHeaderHTML(project) {
  ...
  return `<div class="report-header">
    ...
    <div class="subtitle">${esc(project.production_company)}${project.client ? ` &mdash; ${esc(project.client)}` : ''}</div>
...
function detectBrandStandalone(name) { var n=(name||"").toUpperCase(); var brands={red:["V-RAPTOR","KOMODO","RED ONE","EPIC-X","EPIC-W","SCARLET","RAVEN","WEAPON","MONSTRO","HELIUM","GEMINI","DRAGON","RANGER","DSMC"],arri:["ALEXA","AMIRA","ARRICAM","ARRIFLEX"],blackmagic:["URSA","BMPCC","PYXIS","POCKET CINEMA","BLACKMAGIC"],sony:["VENICE","BURANO","FX3","FX30","FX6","FX9","FR7","A7S","A7R","FS5","FS7","CINEALTA"],canon:["C70","C80","C100","C200","C300","C400","C500","C700","R5 C","R5C","EOS C"],dji:["RONIN","ZENMUSE","INSPIRE","MAVIC"],nikon:["NIKON Z","Z5","Z6","Z7","Z8","Z9"," ZR"],apple:["IPHONE","IPAD"]}; for(var k in brands){if(brands[k].some(function(kw){return n.indexOf(kw)>=0}))return k;} return null; }
function renderCameras(cams) { var typeLabels={camera:"Video",audio:"Audio",photo:"Photo",misc:"Misc Source"}; return '<div class="section"><div class="section-header">Media Sources</div><div class="camera-cards">'+cams.map(function(c){ var type=c.type||"camera"; var brand=detectBrandStandalone(c.name); var brandHtml=brand?'<img class="brand-logo" src="/icons/cameras/'+brand+'.svg" alt="">':''; var specs=''; if(type==="audio"){specs='<div><div class="cam-spec-label">Sample Rate</div><div class="cam-spec-value mono">'+c.res+'</div></div><div><div class="cam-spec-label">Bit Depth</div><div class="cam-spec-value mono">'+c.codec+'</div></div><div><div class="cam-spec-label">Channels</div><div class="cam-spec-value">'+c.fps+'</div></div>';} else if(type==="photo"){specs='<div><div class="cam-spec-label">Format</div><div class="cam-spec-value mono">'+c.codec+'</div></div>';} else if(type==="misc"){specs='<div><div class="cam-spec-label">Resolution</div><div class="cam-spec-value">'+c.res+'</div></div><div><div class="cam-spec-label">Codec</div><div class="cam-spec-value mono">'+c.codec+'</div></div>';} else{specs='<div><div class="cam-spec-label">Resolution</div><div class="cam-spec-value">'+c.res+'</div></div><div><div class="cam-spec-label">Codec</div><div class="cam-spec-value mono">'+c.codec+'</div></div><div><div class="cam-spec-label">Colorspace</div><div class="cam-spec-value mono">'+c.colorspace+'</div></div><div><div class="cam-spec-label">LUT</div><div class="cam-spec-value mono">'+c.lut+'</div></div><div><div class="cam-spec-label">FPS</div><div class="cam-spec-value">'+c.fps+'</div></div>'+(c.audio?'<div><div class="cam-spec-label">Audio</div><div class="cam-spec-value">'+c.audio+'</div></div>':'');} var badge=type!=="camera"?'<span class="cam-label" style="background:var(--accent3)">'+typeLabels[type]+'</span>':''; return '<div class="camera-card">'+brandHtml+'<h3>'+c.name+(c.label?' <span class="cam-label">'+c.label+'</span>':'')+' '+badge+'</h3><div class="cam-specs">'+specs+'</div></div>';}).join('')+'</div></div>'; }
