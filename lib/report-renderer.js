...
:root {
  ...
  --shadow: none;
  --tr-caps: 0.14em;
  /* Categorical palette for per-source data */
  --chart-1: var(--moss); --chart-2: var(--clay); --chart-3: var(--granite);
  --chart-4: var(--madrone); --chart-5: var(--alpenglow); --chart-6: var(--ochre);
}
.theme-light {
  --bg: var(--snow);
  --surface: var(--snow);
...
.theme-film {
  ...
  --chart-1: color-mix(in oklab, var(--moss) 40%, var(--parchment));
  --chart-2: color-mix(in oklab, var(--clay) 75%, var(--parchment));
  --chart-3: color-mix(in oklab, var(--granite) 55%, var(--parchment));
  --chart-4: var(--madrone);
  --chart-5: var(--alpenglow);
  --chart-6: var(--ochre);
}
...
.masthead h1 { font-size: 30px; font-weight: 600; letter-spacing: -0.035em; line-height: 1.02; margin-bottom: 6px; color: var(--parchment); }
...
.day-summary-card h3 { font-size: 18px; font-weight: 500; letter-spacing: -0.015em; margin-bottom: 4px; }
...
.day-source-bar { display: flex; height: 8px; border-radius: 4px; overflow: hidden; margin: 16px 0 10px; }
.day-source-bar span { display: block; height: 100%; }
...
var SRC_COLOR = (function(){ var m = new Map(); DAYS.forEach(function(d){ daySourceSums(d).forEach(function(v, k){ if (!m.has(k)) m.set(k, (m.size % 6) + 1); }); }); return m; })();
